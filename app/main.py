...
def register_user(user: UserCreate, db: psycopg2.extensions.connection = Depends(get_db)):
    with db.cursor() as cursor:
        ...
        cursor.execute(
            ...
            (user.name, user.email, hashed_password, user.role, datetime.utcnow()),
        )
...
@app.post("/login")
def login_user(user: UserCreate, db: psycopg2.extensions.connection = Depends(get_db)):
    with db.cursor() as cursor:
        cursor.execute("SELECT * FROM users WHERE email = %s", (user.email,))
...
# Call the function on startup to create tables
...
def insert_sample_data(db: psycopg2.extensions.connection):
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES
                ('Ноутбук', 'Мощный ноутбук для работы и игр', 50000.00, 10, 2, '{"color": "черный", "processor": "Intel i7", "ram": "16GB"}'),
                ('Смартфон', 'Современный смартфон с отличной камерой', 25000.00, 15, 3, '{"color": "белый", "camera": "12MP", "battery": "4000mAh"}'),
                ('Телевизор', 'Ультра HD телевизор с поддержкой Smart TV', 35000.00, 8, 4, '{"size": "55 inch", "type": "LED", "resolution": "4K"}'),
                ('Футболка', 'Стильная мужская футболка', 1500.00, 50, 6, '{"size": "M", "color": "синий"}'),
                ('Платье', 'Элегантное платье для особых случаев', 3000.00, 30, 7, '{"size": "S", "color": "красный"}'),
                ('Кроссовки', 'Удобные кроссовки для спорта', 4000.00, 25, 9, '{"size": "42", "color": "черный"}'),
                ('Сандалии', 'Летние сандалии для отдыха', 2000.00, 40, 10, '{"size": "38", "color": "бежевый"}'),
                ('Кресло', 'Удобное кресло для офиса', 8000.00, 15, 12, '{"color": "черный", "material": "кожа"}'),
                ('Кровать', 'Комфортная двуспальная кровать с матрасом', 25000.00, 20, 12, '{"material": "дерево", "size": "King"}'),
                ('Игрушечный робот', 'Интерактивный робот для детей', 1500.00, 50, 16, '{"battery": "AA", "color": "красный"}'),
                ('Детская футболка', 'Яркая футболка для детей', 800.00, 60, 17, '{"size": "L", "color": "голубой"}');
        """)
